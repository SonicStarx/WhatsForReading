<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      {{ __('Performance Assessment - Edit existing assessment') }}
    </h2>
  </x-slot>

  <a href="{{ route('individualperformance', $pupilp->pupilid) }}" class="p-5 ml-32 underline text-gray-600 hover:text-gray-900 flex"> Return to  summary </a>
  ...
  <div class=" w-1/2 p-5 m-auto">
    <form method="POST" action=" {{ route('updateassessment', $pupilp->id) }}" enctype="multipart/form-data" >
      @method('PUT')
      @csrf
      ...
      <div class="mt-3">
        ...
        <select id="objectives" name='objectives' class="block mt-1 w-full" >
          ...
          @foreach($LOs as $LO)
          <option class="sortMe" value="{{ $LO->LO_id }}" {{$LO->LO_id == $pupilp->LO_id ? 'selected' : ''}}> [{{ $LO->year_group }}] {{ $LO->LO_title }} </option>
          @endforeach
        </select>
      </div>
      ...
      <div class=" mt-3">
        ...
        <select id="performance" name='performance' class="block mt-1 w-full" value="{{$pupilp->performance}}">
          <option value="">Select current progress level</option>
          @if($pupilp->performance == "Working above expectations")
          ...
          @elseif($pupilp->performance == "Working at expectations")
          ...
          @elseif($pupilp->performance == "Working towards expectations")
          <option value="Working above expectations" >Working above expectations (Greater Depth)</option>
          <option value="Working at expectations" >Working at expectations</option>
      ...
      <div class="mt-8">
        ...
        <x-input id="assessment_date" class="block mt-1 w-full" type="date" name="assessment_date" value="{{ $pupilp->assessment_date->toDateString()}}" required autofocus />
      </div>
